...
def Register(root,frame1):
    ...
    Label(frame1, text="Dept:              ", anchor="e", justify=LEFT).grid(row = 4,column = 0)
    Label(frame1, text="Regsister:           ", anchor="e", justify=LEFT).grid(row = 5,column = 0)
    Label(frame1, text="EmailID:             ", anchor="e", justify=LEFT).grid(row = 6,column = 0)
    ...
    reg = Button(frame1, text="Submit", command = lambda: reg_server(root, frame1, name.get(), sex.get(), zone.get(), city.get(), password.get()), width=10)
